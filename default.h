...
typedef struct _line_tag {
	...
	int line_count;				// 라인 내부의 문자 수
	struct _line_tag * down;		// 아래 라인 주소
	struct _line_tag * up;			// 윗 라인 주소
	struct _char_tag * char_point;	// 라인 맨 처음 문자 주소
} stLine;
...
#define OO_LINE_D		207
#define UU_LINE_D		208
...
// Save&Load Dialog 관련
#define DIALOG_W			34			// dialog width
...
#define DIALOG_PATH_LEN	14

#define POS_DIALOG_X			23		// dialog left-up  x-axis
#define POS_DIALOG_Y			7		// dialog left-up y-axis
#define POS_PATH_X				25		// path window left-up y-axis
#define POS_PATH_Y				9		// path window left-up y-axis


// Save&Load 관련 다이얼로그 좌표
#define DIALOG_BLANK_W			((int)((DIALOG_PATH_W-DIALOG_BTN_LEN*2)/3))
																	// Save&Load x-axis
...
// New 관련 다이얼로그 좌표
#define DIALOG_BLANK_W2		((int)((DIALOG_PATH_W-DIALOG_BTN_LEN*2-DIALOG_BTN_LEN2)/4))
																	// Save x-axis
...
#define LINE_ALIGN(stTmpL)		while(stTmpL != 0)	\
								{	\
									if(stTmpL->up == 0)	stTmpL->line_number = 1;\
									else	stTmpL->line_number = (stTmpL->up)->line_number + 1;	\
									stTmpL = stTmpL->down;	\
								}

#define CHAR_ALIGN(stTmpC)		while(stTmpC != 0)	\
								{	\
									if(stTmpC->prev == 0)		stTmpC->char_number = 1;\
									else		stTmpC->char_number = (stTmpC->prev)->char_number + 1;	\
									stTmpC = stTmpC->next;	\
								}
