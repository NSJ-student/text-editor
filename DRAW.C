...
void Draw_MainWindow(void)
{
	...
	Draw_VLine(56, 0, 3);
	Put_Str(58, 	POS_MENU_Y, "line-char : ");
...
{
	VGA_CHAR(x, y, UU_LINE);
	for(y++, height = y+height-1; y<height-1; y++)
	{
		VGA_CHAR(x, y, V_LINE);
	}
	VGA_CHAR(x, y, OO_LINE);
}
...
void Clr_Text()
{
	...
	for(y=0; y<TEXT_H; y++)
	{
		for(x = 0; x<TEXT_W; x++)
		{
			VGA_CHAR(POS_TEXT_X+x, POS_TEXT_Y+y, 0);
		}
	}
}
...
void Clr_Dialog()
{
	...
	for(j = 0; j<DIALOG_H-2; j++)
	{
		...
		{
			VGA_CHAR(POS_PATH_X-1+i, POS_PATH_Y+j, 0);
		}
	}
}
